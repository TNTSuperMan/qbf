...
pub fn ir_to_bytecodes(ir_nodes: &[IR], range_info: &RangeInfo) -> Result<Vec<Bytecode>> {
    ...
    loop {
        match ir_nodes.get(i) {
            ...
            Some(node) => {
                let delta = i16::try_from(node.pointer.wrapping_sub(last_ptr))?;
                ...
                match &node.opcode {
                    ...
                    IROp::Add(val1) => {
                        match ir_nodes[i + 1] {
                            IR { opcode: IROp::Add(val2), pointer: ptr2 } => {
                                let delta2 = i16::try_from(ptr2 - last_ptr)?;
                                last_ptr = ptr2;
                        ...
                        if let Ok(step_i8) = i8::try_from(*step) {
                            match ir_nodes[i + 1] {
                                IR { opcode: IROp::Add(val), pointer: ptr } => {
                                    let delta2 = i8::try_from(ptr - last_ptr)?;
                                    last_ptr = ptr;
                                    match mid_range {
                    ...
                    IROp::MulAndSetZero(dests) => {
                        ...
                        for (dest_ptr, dest_val) in dests {
                            bytecodes.push(Bytecode::Mul { delta: i16::try_from(dest_ptr.wrapping_sub(last_ptr))?, val: *dest_val });
                        }
                    }
                    IROp::MovesAndSetZero(dests) => {
                        if let [(p1, f1), (p2, f2)] = dests.iter().as_slice() {
                            let delta1 = i16::try_from(p1.wrapping_sub(last_ptr))?;
                            let delta2 = i16::try_from(p2.wrapping_sub(last_ptr))?;
                            ...
                            for (dest_ptr, is_pos) in dests {
                                if *is_pos {
                                    bytecodes.push(Bytecode::MoveAdd {
                                        delta: i16::try_from(dest_ptr.wrapping_sub(last_ptr))?,
                                    });
                                } else {
                    ...
                    IROp::LoopEnd(_start) => {
                        ...
                        if let Bytecode::JmpIfZero { addr_abs: addr, .. } = &mut bytecodes[start] {
                            *addr = (end + 1) as u32;
                            ...
                            bail!("InternalError: Corresponding JmpIfZero is not hit");
                        }
                    ...
                    IROp::LoopEndWithOffset(_start, offset) => {
                        ...
                        match range {
                            ...
                            MidRange::Both(range) => bytecodes.push(Bytecode::BothRangeCheckJNZ { delta: i8::try_from(delta)?, addr_back: subrel as u16, range: range.clone() }),
                        }
                    }
