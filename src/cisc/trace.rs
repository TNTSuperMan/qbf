...
pub fn write_trace(vm: &VM, insts: &[Bytecode]) -> Result<()> { Ok(()) }

#[cfg(feature = "debug")]
pub fn write_trace(vm: &VM, insts: &[Bytecode]) -> Result<()> {
    use std::fs;
    use crate::cisc::trace::generate_bytecode_trace;

    fs::write("./box/bytecodes", generate_bytecode_trace(&insts, &vm.ocm))?;
    fs::write("./box/memory", *vm.memory.0)?;
