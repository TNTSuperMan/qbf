...
pub fn run_deopt(vm: &mut VM) -> Result<InterpreterResult, String> {
    ...
    loop {
        ...
        match bytecode.opcode {
            ...
            OpCode::Mul => {
                vm.memory.add(vm.pointer.wrapping_add_signed(bytecode.delta as isize), mul_val.wrapping_mul(bytecode.val))?;
            }
            ...
            OpCode::SingleMoveAdd => {
                ...
                if v != 0 {
                    vm.memory.set(vm.pointer, 0)?;
                    vm.memory.add(vm.pointer.wrapping_add_signed(bytecode.addr as i32 as isize), v)?;
                }
            }
            OpCode::SingleMoveSub => {
                ...
                if v != 0 {
                    vm.memory.set(vm.pointer, 0)?;
                    vm.memory.sub(vm.pointer.wrapping_add_signed(bytecode.addr as i32 as isize), v)?;
                }
            }
            ...
            OpCode::DoubleMoveAddAdd => {
                ...
                if v != 0 {
                    ...
                    vm.memory.add(vm.pointer.wrapping_add_signed(d1 as isize), v)?;
                    vm.memory.add(vm.pointer.wrapping_add_signed(d2 as isize), v)?;
                    vm.memory.set(vm.pointer, 0)?;
                }
            }
            OpCode::DoubleMoveAddSub => {
                ...
                if v != 0 {
                    let (d1, d2) = u32_to_two_delta(bytecode.addr);
                    vm.memory.add(vm.pointer.wrapping_add_signed(d1 as isize), v)?;
                    vm.memory.sub(vm.pointer.wrapping_add_signed(d2 as isize), v)?;
                    vm.memory.set(vm.pointer, 0)?;
                }
            }
            OpCode::DoubleMoveSubAdd => {
                ...
                if v != 0 {
                    let (d1, d2) = u32_to_two_delta(bytecode.addr);
                    vm.memory.sub(vm.pointer.wrapping_add_signed(d1 as isize), v)?;
                    vm.memory.add(vm.pointer.wrapping_add_signed(d2 as isize), v)?;
                    vm.memory.set(vm.pointer, 0)?;
                }
            }
            ...
            OpCode::MoveAdd => {
                vm.memory.add(vm.pointer.wrapping_add_signed(bytecode.delta as isize), mul_val)?;
            }
            OpCode::MoveSub => {
                vm.memory.sub(vm.pointer.wrapping_add_signed(bytecode.delta as isize), mul_val)?;
            }
