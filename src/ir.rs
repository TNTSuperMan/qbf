...
pub fn parse_to_ir(code: &str) -> Result<Vec<IR>> {
    ...
    for char in code.chars() {
        match char {
            ...
            ']' => {
                let start = loop_stack.pop().with_context(|| "Syntax Error: Unmatched closing bracket")?;
    ...
    if loop_stack.len() != 0 {
        bail!("Syntax Error: Unmatched opening bracket");
    }
